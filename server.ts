...
app.post("/api/recipes/generate", async (req, res) => {
  ...
  const systemPrompt = `You are a professional pastry chef and master of sugar-free culinary arts. You specialize in crafting delicious desserts and beverages using Monkaura (an Erythritol and Monk Fruit sweetener blend). Monkaura is unique because it replaces sugar 1:1 with clean, natural sweetness, zero added sugar, and zero net calories. You must always output responses strictly in the requested JSON structure.`;

  const userPrompt = `Create a delicious, creative, and fully low-sugar/low-carb recipe.
The recipe MUST focus on using or incorporating "${ingredient.trim()}" as a primary ingredient, and MUST use "Monkaura Erythritol & Monk Fruit Blend" as the exclusive sugar-free sweetener.
Category of recipe: ${category}. Sweetener usage style: ${preference}.

Ensure the recipe is 100% realistic, has accurate measurements, is safe for low-sugar diets, and is delicious.
In the ingredients list, include where "Monkaura Sweetener" is used (e.g. "3 tbsp Monkaura Erythritol & Monk Fruit Blend").
Include clear step-by-step instructions.
Provide precise nutritional info per serving, and highlight how Monkaura saves sugar/calories.
