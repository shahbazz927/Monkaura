...
export default function FloatingWhatsApp() {
  ...
  const handleClick = () => {
    const text = encodeURIComponent("Hi! I'm visiting the Monkaura website and would like to learn more about ordering your Allulose & Monk Fruit sweetener blend.");
    window.open(`https://api.whatsapp.com/send?phone=919391774374&text=${text}`, "_blank");
  };

  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2 pointer-events-none">
      {/* Friendly interactive Tooltip */}
      {showTooltip && (
        <div className="pointer-events-auto bg-white text-brand-dark px-4 py-2.5 rounded-2xl shadow-xl border border-gray-100 flex items-center gap-2 max-w-[250px] animate-fadeIn text-xs relative select-none">
      ...
      <button
        onClick={handleClick}
        className="pointer-events-auto w-14 h-14 bg-[#25D366] hover:bg-[#20ba5a] text-white rounded-full shadow-2xl flex items-center justify-center cursor-pointer hover:scale-108 active:scale-95 transition-all duration-300 relative group"
